...
void draw()
{
    ...
    int posY = 0;
    bool YDone;
    while(posY <= SCREEN_HEIGHT && posX <= SCREEN_WIDTH){
        YDone = (posY == SCREEN_HEIGHT);
        if(!YDone) {
        SDL_SetRenderDrawColor(gRenderer, 0x00 /*R*/, 0x00 /*G*/, 0x00 /*B*/, 0xFF /*A*/);
        SDL_RenderDrawLine(gRenderer, posX, posY, centerX, centerY);

        SDL_SetRenderDrawColor(gRenderer, 0x00 /*R*/, 0x00 /*G*/, 0xFF /*B*/, 0xFF /*A*/);
        SDL_RenderDrawLine(gRenderer, SCREEN_WIDTH-posX, SCREEN_HEIGHT-posY, centerX, centerY);
        posY += 20;
    }
    else{
